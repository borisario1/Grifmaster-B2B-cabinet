...
test('при создании профиля все настройки уведомлений включены по умолчанию', function () {
...
test('каждая настройка уведомлений корректно переключается', function (string $settingName) {
...
test('система запрещает отключение защищенных уведомлений (от менеджера)', function () {
    $user = User::factory()->create();
    UserProfile::create(['user_id' => $user->id]);
